...
=== Inspecting abbreviation resolution

For complex projects, it might not be obvious if the correct task was executed. When using abbreviated tasks, a single typo can easily change which task is running.

When INFO, or <<logging.adoc#logLevels, more verbose logging is enabled>>, the logs will contain extra information about the project, and task resolution. For example, when executing the `mAL:cT` command on the previous example, the following log messages will be visible:
